...
const StickyCard = styled(Box)<{ stackIndex: number; isLast?: boolean; isSecondLast?: boolean }>(
  ({ stackIndex, isLast, isSecondLast }) => ({
    position: isLast ? "relative" : "sticky",
    top: `calc(${STICKY_HEADER_HEIGHT}px + ${
      stackIndex * CARD_STACK_OFFSET
    }px)`,
    zIndex: stackIndex + 1,
    // Last card: no padding, Second-to-last: reduced padding, Others: full padding
    paddingBottom: isLast ? "0" : isSecondLast ? "80px" : "50vh",
      ...
      <Container maxWidth="lg">
        ...
        <StackingContainer>
          {processSteps.map((step, index) => {
            ...
            const isSecondLast = index === processSteps.length - 2;
